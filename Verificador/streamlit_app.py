...
from Lista_Agentes import obtener_agentes
...
def procesar_enlace(link, url_docs, preguntas_generadas, API_KEY, ORG_ID):
...
def main():
    ...
    prompt = obtener_prompt_agente(agent_id,API_KEY, ORG_ID)
    if prompt:
        with st.expander("Agent Prompt Analysis"):
            st.write("Agent Prompt:")
            st.text(prompt)
            analisis = analizar_prompt(prompt, API_KEY, ORG_ID)
            if analisis:
    ...
    if st.button("Analyze"):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(procesar_enlace, link, url_docs, preguntas_generadas, API_KEY, ORG_ID) for link in links]
            ...
            for future in as_completed(futures):
                ...
                if result:
                    ...
                    with st.expander(f"Question {len(preguntas_generadas)}"):
                        ...
                        evaluacion_estructura = evaluar_estructura(prompt, respuesta, pregunta, API_KEY, ORG_ID)
        ...
        if resultados:
            ...
            tiempo_promedio = df_resultados['Response Time (s)'].mean()
            st.write(f"Tiempo de respuesta promedio: {tiempo_promedio:.2f} segundos")

            # Calcular el puntaje promedio de estructura
            puntajes_estructura = df_resultados['Structure Score'].apply(lambda x: int(x.split('/')[0]) / int(x.split('/')[1]) if x != 'N/A' else 0)
